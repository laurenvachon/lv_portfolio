 ...
 * Lauren Vachon
 */
jQuery(document).ready(function(d){var e;var c=0;var g=5;var b=d("header").outerHeight();d(window).scroll(function(h){e=true});setInterval(function(){if(e){a();e=false}},50);function a(){var h=d(this).scrollTop();if(Math.abs(c-h)<=g){return}if(h>c&&h>b){d("header").removeClass("nav-down").addClass("nav-up")}else{if(h+d(window).height()<d(document).height()){d("header").removeClass("nav-up").addClass("nav-down")}}c=h}AOS.init({duration:1200,});if(d("body").hasClass("homepage")){var f=d("#section1").height();d(window).scroll(function(){if(d(this).scrollTop()>f-400){d(".navbar").css("right","0");d(".navbar").addClass("moveOut")}else{d(".navbar").css("right","-80px");d(".navbar").removeClass("moveOut")}});d(".soSmooth").on("click",function(h){if(this.hash!==""){h.preventDefault();var i=this.hash;d("html, body").animate({scrollTop:d(i).offset().top},800,function(){window.location.hash=i})}})}if(d("body").hasClass("journal")){d(function(){var h=d("#jsonContent");var i={rss_url:"https://medium.com/feed/@laurenvachon"};d.get("https://api.rss2json.com/v1/api.json",i,function(k){if(k.status=="ok"){var j="";var l=d.map(k.items,function(o,n){var m=k.items[n].categories;if(m.length!==0){return o}});d.each(k.items,function(q,w){var p;if(q<12){p=""}else{p=" visible-sm"}j+='<div class="col-sm-6 col-md-4'+p+'">';j+='<div class="blog-post"><div>';var t=w.description.indexOf("<img");var u=w.description.substring(t).indexOf("src=")+t;var r=u+5;var v=w.description.substring(r).indexOf('"')+r;var m=w.description.substring(r,v);j+='<div class="blog-element"><img class="img-responsive" src="'+m+'" width="360px" height="240px"></div></div>';j+='<div class="blog-content"><h3>'+w.title+"</h3>";j+='<div class="post-meta"><span>By '+w.author+"</span></div>";var o=w.description.replace(/<img[^>]*>/g,"");var n=120;var s=o.substr(0,n);s=s.substr(0,Math.min(s.length,s.lastIndexOf(" ")));j+="<p>"+s+"...</p>";j+='<p><a href="'+w.link+'" target="_blank"> Continue Reading </a></p>';j+="</div></div></div>";return q<12});h.html(j)}})})}if(d("body").hasClass("page-template-page-photography")){d("img").on("contextmenu",function(h){return false});d(window).load(function(){d(".grid").fadeIn(3000).masonry({itemSelector:".grid-item",columnWidth:".grid-sizer",percentPosition:true})});d("[data-fancybox]").fancybox({speed:630,transitionEffect:"fadeIn",image:{protect:true}})}});